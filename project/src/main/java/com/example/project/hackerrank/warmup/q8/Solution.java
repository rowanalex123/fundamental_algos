package com.example.project.hackerrank.warmup.q8;
...
public class Solution {
    ...
    private void run() throws Exception {
        ...
        int len = in.nextInt();
        List<Long> inp = new ArrayList<>(len);
        forn(len, index -> inp.add(in.nextLong()));
        inp.sort(Long::compareTo);
        Collections.reverse(inp);
        long opCnt = 0, lastVal = Long.MIN_VALUE;
        for (long val : inp) {
            if (lastVal == Long.MIN_VALUE) {
                lastVal = val;
                opCnt++;
                continue;
            }
            if (lastVal == val) opCnt++;
            else break;
        }
        op.out("{}",opCnt);
