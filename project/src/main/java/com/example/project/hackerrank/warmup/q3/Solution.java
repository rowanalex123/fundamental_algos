package com.example.project.hackerrank.warmup.q3;
...
public class Solution {
    ...
    private void run() throws Exception {
        int[] aRatings = IntStream.rangeClosed(1, 3)
                .map(o -> in.nextInt())
                .toArray();
        int[] bRatings = IntStream.rangeClosed(1, 3)
                .map(o -> in.nextInt())
                .toArray();
        int aScore = 0, bScore = 0;
        for (int i = 0; i < 3; i++) {
            if (aRatings[i] < bRatings[i]) bScore++;
            else if(aRatings[i] > bRatings[i]) aScore++;
        }
        op.log("inp: {}, {}",aRatings,bRatings);
        op.out("{} {}", aScore,bScore);
    }
