package com.example.project.hackerrank.warmup.q4;
...
public class Solution {
    ...
    private void run() throws Exception {
        int matrixLen = in.nextInt();
        int[][] inp = new int[matrixLen][matrixLen];

        for (int j = 0; j < matrixLen; j++)
            for (int i = 0; i < matrixLen; i++) {
                int matVal = in.nextInt();
                inp[i][j] = matVal;
            }
        op.log("Got input matrix: {}", (Object) inp);


        int sumLeftToRight = 0, sumRightToLeft = 0, index = 0;

        //left to right
        while (index < matrixLen) {
            sumLeftToRight += inp[index][index];
            index++;
        }
        ...
        index = matrixLen - 1;
        while (index >= 0) {
            sumRightToLeft += inp[matrixLen - index - 1][index];
            index--;
        }

        op.out("{}", Math.abs(sumLeftToRight - sumRightToLeft));
    }
    ...
    static final Scanner in = new Scanner(new BufferedInputStream(devMode ? executeQuitely(() -> new FileInputStream("input.txt")) : System.in), "ISO-8859-1");
     ...
     */
    class OutputHelper {
        private Writer outputFile = devMode ? executeQuitely(() -> new BufferedWriter(new FileWriter("output.txt"))) : null;
        ...
        public boolean endJob() {
            ...
            return executeQuitely(() -> {
                ...
                BufferedReader actual = new BufferedReader(new FileReader("output.txt"));
